...
const ARENA_W = 1200; // Much wider for more reaction time
const ARENA_H = 600;
const PADDLE_W = 16;
const PADDLE_H = 120;
const BALL_SIZE = 16;
...
const playSound = (type: 'hit' | 'wall' | 'score' | 'countdown') => {
  try {
...
export function Pong({ local, channel, isHost }: { local: boolean, channel?: any, isHost?: boolean }) {
  ...
  const s = useRef({
    ball: { x: ARENA_W / 2, y: ARENA_H / 2, vx: 0, vy: 0, speed: 5 },
  ...
  const updateDOM = useCallback(() => {
    ...
    // Hide ball completely if it's placed offscreen (e.g. during an explosion)
    if (s.current.ball.x < -500 || s.current.ball.x > ARENA_W + 500) {
  ...
  const launchBall = useCallback(() => {
    ...
    s.current.ball = {
      ...
      y: ARENA_H / 2,
      speed: baseSpeed,
      vx: (dirX / len) * baseSpeed,
      vy: (dirY / len) * baseSpeed,
    };
  ...
  const triggerLaunchSequence = useCallback(() => {
    let count = 3;
    setCountdown(count);
    playSound('countdown');
    
    // Reset ball visually to center immediately
    s.current.ball = { x: ARENA_W / 2, y: ARENA_H / 2, vx: 0, vy: 0, speed: 0 };
    updateDOM();

    if (!local && isHost && channel) channel.send({ type: 'broadcast', event: 'pong_countdown', payload: count });
  ...
  }, [local, isHost, channel, launchBall, updateDOM]);

  const handleScore = useCallback((player: 'host' | 'guest') => {
    playSound('score');
    s.current.playing = false; // Freeze the game engine
    
    // Setup visual explosion where the ball crossed the line
    const explodeX = player === 'host' ? ARENA_W : 0;
    const exp = { x: explodeX, y: s.current.ball.y, color: player === 'host' ? '#f43f5e' : '#3b82f6' };
    setExplosion(exp);
    setTimeout(() => setExplosion(null), 1000);
    
    // Hide ball immediately so it doesn't float
    s.current.ball.x = -1000;
    updateDOM();
    ...
    setScores(prev => {
      ...
      if (newScores[player] >= MAX_SCORE) {
        ...
        setTimeout(() => {
          if (!s.current.playing && !winner) triggerLaunchSequence();
        }, 1500);
      }
      ...
      if (!local && isHost && channel) {
        channel.send({ 
          type: 'broadcast', 
          event: 'pong_score', 
          payload: { scores: newScores, winner: newScores[player] >= MAX_SCORE ? player : null, explosion: exp } 
        });
      }
      return newScores;
    });
  }, [local, isHost, channel, triggerLaunchSequence, winner, updateDOM]);
  ...
  const handlePaddleHit = (paddleY: number) => {
    ...
    b.vx *= -1;
    // Gentler speed scaling, lower cap
    b.speed = Math.min(b.speed * 1.05, 14); 
    
    let relativeY = (b.y + BALL_SIZE / 2) - (paddleY + PADDLE_H / 2);
    let normalizedRelativeY = relativeY / (PADDLE_H / 2); // -1 to 1
    let bounceAngle = normalizedRelativeY * (Math.PI / 4);
  ...
  useEffect(() => {
    ...
    const s1 = channel.on('broadcast', { event: 'pong_countdown' }, ({ payload }: any) => {
    ...
    const s2 = channel.on('broadcast', { event: 'pong_launch' }, ({ payload }: any) => {
      s.current.ball = payload;
      s.current.playing = true;
      setGameStarted(true);
    });
    const s3 = channel.on('broadcast', { event: 'pong_ball_sync' }, ({ payload }: any) => {
      s.current.ball = payload;
      playSound('hit');
    });
    const s4 = channel.on('broadcast', { event: 'pong_paddle' }, ({ payload }: any) => {
      if (payload.player === 'host' && !isHost) s.current.hostY = payload.y;
      if (payload.player === 'guest' && isHost) s.current.guestY = payload.y;
    });
    const s5 = channel.on('broadcast', { event: 'pong_score' }, ({ payload }: any) => {
      if (!isHost) {
        playSound('score');
        s.current.playing = false;
        
        if (payload.explosion) {
          setExplosion(payload.explosion);
          setTimeout(() => setExplosion(null), 1000);
          s.current.ball.x = -1000;
          updateDOM();
        }
    ...
    const s6 = channel.on('broadcast', { event: 'pong_req_restart' }, ({ payload }: any) => {
      setRestartRequestedBy(payload.by);
    });
    const s7 = channel.on('broadcast', { event: 'pong_acc_restart' }, () => {
      executeRestart();
    });
    const s8 = channel.on('broadcast', { event: 'pong_dec_restart' }, () => {
      setRestartRequestedBy(null);
    });

    return () => {};
  }, [local, channel, isHost, updateDOM]);
  ...
  }, [local, isHost, triggerLaunchSequence, updateDOM]);
  ...
  const toggleFullscreen = () => {
    ...
    if (!document.fullscreenElement) {
      arenaRef.current.requestFullscreen().catch(err => console.error(err));
    } else {
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-4 md:p-8 bg-card rounded-3xl border border-border shadow-md w-full max-w-5xl mx-auto">
      ...
      <div 
        ...
        style={{ width: '100%', aspectRatio: '2/1', maxHeight: '70vh' }}
      >
        <button 
          onClick={toggleFullscreen} 
          className="absolute top-4 right-4 z-40 text-white/30 hover:text-white bg-white/5 hover:bg-white/10 p-2 rounded-lg backdrop-blur-sm transition-all"
          title="Fullscreen"
        >
          <Maximize2 size={20} />
        </button>
        ...
        <AnimatePresence>
          {countdown !== null && (
            <motion.div 
              ...
              className="absolute inset-0 flex items-center justify-center pointer-events-none z-30 text-white font-black text-8xl md:text-9xl italic drop-shadow-[0_0_20px_currentColor]"
            >
              {countdown}
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {explosion && (
            <motion.div
              ...
              style={{
                left: `${(explosion.x / ARENA_W) * 100}%`,
                top: `${(explosion.y / ARENA_H) * 100}%`,
                width: 160,
                height: 160,
                marginLeft: -80,
                marginTop: -80,
                ...
                boxShadow: `0 0 40px ${explosion.color}, inset 0 0 20px white`,
              }}
            />
          )}
        </AnimatePresence>
          ...
          {winner && (
            ...
            >
              ...
              >
                ...
                <h2 className={`text-4xl md:text-5xl font-black uppercase tracking-widest drop-shadow-[0_0_20px_currentColor] ${winner === 'host' ? 'text-blue-500' : 'text-rose-500'}`}>
      ...
      <div className="flex w-full items-center justify-between mt-2 max-w-lg px-4">
        ...
        <Button 
          variant="secondary" 
          className="font-black uppercase tracking-widest px-6 md:px-8"
