// Shared logic for relaying third-party video-embed/CDN requests, forging Referer/
// Origin to whatever the actual requesting page's URL would be for that hop.
//
// This exists ONLY because these embed hosts and their CDNs refuse to serve video
// unless the request's Referer looks right, and the operator explicitly chose — after
// being told this conflicts with this project's original "never spoof/forge headers,
// never bypass anti-hotlink/access-control checks" rule — to override that rule
// specifically for this. See buildEmbedUpstreamHeaders in api/_proxy/config.ts.
//
// It's a *factory* (createEmbedHandler) rather than a single fixed handler because it
// runs in two different places that need different link bases:
//  - api/embed.ts: Vercel Edge Function, links point at /api/embed
//  - relay/server.ts: a plain Node process (intended to run on the operator's own home
//    network, reached through a tunnel), links point at /embed and require a shared
//    key. This second path exists specifically because this particular video provider
//    blocks requests from cloud/datacenter IP ranges (a DIFFERENT anti-bot mechanism
//    than the Referer check, discovered by testing) — the operator explicitly chose to
//    relay these specific requests through their own real home connection rather than
//    Vercel's, understanding that still means routing around a block the provider put
//    up on purpose, just with a real IP instead of a forged identity.
//
...
// carries pageRef = the page it was discovered on, pinned to the current *document*
// rather than advancing per-resource (matching how browsers actually compute Referer
// for player-issued sub-requests).
//
...
// only, private/loopback hostnames blocked (more important than ever when this runs on
// a home network — this is what stops the relay being turned into SSRF against the
// operator's own LAN), rate-limited, the "ref" site key must be one of the explicitly
// configured PROXY_SITES, and — when a key is required (the home relay) — the caller
// must present it.

import { findSite, checkRateLimit, buildEmbedUpstreamHeaders, buildDownstreamHeaders } from './config';
...
export type EmbedHandlerOptions = {
  /** Where this handler's own generated links point, e.g. "/api/embed" or "/embed". */
  basePath: string;
  /** If set, every request (including self-generated follow-up links) must carry this
   *  as `?k=`. Used by the home relay, since it's reachable from the public internet. */
  requireKey?: string;
};

export function createEmbedHandler({ basePath, requireKey }: EmbedHandlerOptions) {
  function embedLink(refKey: string, target: string, pageRef: string): string {
    const key = requireKey ? `&k=${encodeURIComponent(requireKey)}` : '';
    return `${basePath}?ref=${refKey}&pageRef=${encodeURIComponent(pageRef)}&u=${encodeURIComponent(target)}${key}`;
  }

  // Rewrites absolute http(s) URLs found in quoted strings. Relative URLs constructed
  // at runtime by a script are a known miss — same caveat as the main proxy's rewrite.
  ...
  // HLS (.m3u8) playlists are plain text, line-based: '#EXT-*' directives (some of
  // which carry a quoted URI="...") and bare URL/path lines for segments/sub-playlists.
  ...
  // Finds every `eval(function(p,a,c,k,e,d){...}(...))` block, unpacks it, rewrites
  // URLs found in the unpacked source, and splices the plain (unpacked) code back in
  // place of the eval(...) wrapper — the browser then just runs ordinary JS.
  function unpackAndRewriteEvalBlocks(src: string, refKey: string, pageRef: string): string {
    ...
    while (true) {
      ...
      const outerOpenParen = evalIdx + 4;
  ...
  return async function handleEmbedRequest(req: Request): Promise<Response> {
    ...
    if (requireKey && url.searchParams.get('k') !== requireKey) {
      return new Response('Not found', { status: 404, headers: { 'x-proxy-reason': 'bad-key' } });
    }
